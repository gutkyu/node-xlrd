...
exports._cell_opcode_dict = {};
exports._cell_opcode_list.forEach(function(x){ exports._cell_opcode_dict[exports._cell_opcode] = 1;});
...
//unpack_string
exports.unpack_string = function(data,pos,encoding,lenlen){
	lenlen == lenlen || 1;
	if(lenlen >2) throw new Error('decode error : lenlen ',lenlen,' > 2');
	var len = lenlen == 1? data.readUInt8(pos):data.readUInt16LE(pos);
    pos += lenlen;
	return decode( data,pos,len,encoding);
}

exports.unpack_string_update_pos = function(data, pos, encoding, lenlen, known_len){
	lenlen = lenlen || 1;
	known_len = known_len || null;
	var nchars = 0;
    if (known_len)
        // On a NAME record, the length byte is detached from the front of the string.
        nchars = known_len;
    else{
        nchars = lenlen == 1? data.readUInt8(pos):data.readUInt16LE(pos);
        pos += lenlen;
	}
    newpos = pos + nchars;
    return [encoding(data, pos, nchars,encoding), newpos];
}
exports.unpack_unicode = function(data, pos, lenlen){
	lenlen = lenlen || 2;
    nchars = lenlen == 1? data.readUInt8(pos):data.readUInt16LE(pos);
	var str = null;
    if (! nchars)
...
/*
exports.function unpack_unicode_update_pos(data, pos, lenlen=2, known_len=None){
...
exports.function biff_dump(mem, stream_offset, stream_len, base=0, fout=sys.stdout, unnumbered=False){
    pos = stream_offset
...
exports.function biff_count_records(mem, stream_offset, stream_len, fout=sys.stdout){
    pos = stream_offset
    stream_end = stream_offset + stream_len
    tally = {}
    while stream_end - pos >= 4:
...
*/
exports.encoding_from_codepage = {
    1200 : 'utf16le',
    10000: 'mac_roman',

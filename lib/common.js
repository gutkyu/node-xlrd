...
exports.cellOpcodeList.forEach(function (x) {
  exports.cellOpcodeDic[exports.cellOpcode] = 1;
});
...
exports.unpackString = function (data, pos, encoding, lenRecordLength) {
  ...
  if (lenRecordLength > 2)
    throw new Error('decode error : lenRecordLength ', lenRecordLength, ' > 2');
  var len = lenRecordLength == 1 ? data.getU8(pos) : data.getU16L(pos);
  pos += lenRecordLength;
  return data.getStr(encoding, pos, pos + len);
};

exports.unpackStringUpdatePos = function (
  data,
  pos,
  encoding,
  lenRecordLength,
  ...
  if (knownLength)
    ...
    nchars = lenRecordLength == 1 ? data.getU8(pos) : data.getU16L(pos);
    pos += lenRecordLength;
  }
  newpos = pos + nchars;
  return [data.getStr(encoding, pos, pos + nchars), newpos];
};
exports.unpackUnicode = function (data, pos, lenRecordLength) {
  lenRecordLength = lenRecordLength || 2;
  var nchars = lenRecordLength == 1 ? data.getU8(pos) : data.getU16L(pos);
  var str = null;
  if (!nchars)
  ...
  if (options & 0x01) {
    ...
    str = data.getStr('utf16le', pos, pos + 2 * nchars);
    ...
    return data.getStr('latin1', pos, pos + nchars);
    // pos += nchars
  }
  // if richtext:
  //     pos += 4 * rt

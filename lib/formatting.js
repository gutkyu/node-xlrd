...
function isDateFormatString(fmt) {
  ...
  if (self._options.verbosity >= 4)
    console.log(
      util.format('isDateFormatString: reduced format is %s', REPR(s))
    );
  s = s.replace(fmtBracketed, '');
  if (nonDateFormats[s]) return false;
  state = 0;
  var separator = ';';
  var gotSep = 0;
  var numCount = 0;
  ...
  for (i = 0; i < s.length; i++) {
    var c = s[i];
    if (dateCharDict[c]) dateCount += dateCharDict[c];
    else if (numCharDict[c]) numCount += numCharDict[c];
    else if (c == separator) gotSep = 1;
  }
  // print numCount, dateCount, repr(fmt)
  if (dateCount && !numCount) return true;
  if (numCount && !dateCount) return false;
  if (dateCount)
    if (self._options.verbosity)
      console.log(
        util.format(
      ...
      if (self._options.verbosity)
        console.log(
          util.format('WARNING *** format %s produces constant result\n', fmt)
        );
...
function handleFormat(data, recType) {
  ...
  if (recType == comm.XL_FORMAT2) bv = Math.min(bv, 30);
  if (!self.encoding) self.deriveEncoding();
  var strPos = 2;
  var fmtKey = null;
  if (bv >= 50) fmtKey = data.getU16L(0);
  else {
  ...
  var uniStrg = null;
  if (bv >= 80) uniStrg = comm.unpackUnicode(data, 2);
  else uniStrg = comm.unpackString(data, strPos, self.encoding, 1);
  var blah = DEBUG || self._options.verbosity >= 3;
  if (blah)
    console.log(
      util.format(
  ...
  if (blah)
    fmtObj.dump(
      util.format('--- handleFormat [%d] ---', self.actualFmtCount - 1)
    );
...
function handleXf(data) {
  ...
  if (bv >= 80) {
    xf.formatKey = data.getU16L(2);
    var pkdTypePar = data.getU16L(4);
    ...
    xf.formatKey = data.getU8(1);
    var pkdTypePar = data.getU8(2);
    xf.is_style = (pkdTypePar & 0x0004) >> 2;
    xf.parent_style_index = (pkdTypePar & 0xfff0) >> 4;
    //todo ���߿�
    /*
    ...
    xf.formatKey = data.getU8(2) & 0x3f;
    ...
    /*
        ...
        for attr_stem in \
            ...
            setattr(xf, attr, 1)
		*/
  } else throw new Error('programmer stuff-up: bv=' + bv);

  xf.xfIndex = self.xfList.length;
  self.xfList.push(xf);
  self.xfCount += 1;
  if (blah) xf.dump(util.format('--- handleXf: xf[%d] ---', xf.xfIndex));

  var fmt = null;
  var cellTyp = null;
  if (
    (fmt = self.formatMap[xf.formatKey]) == undefined ||
    fmt.type == undefined ||
...
function xfEpilogue() {
  ...
  var blah = DEBUG || self._options.verbosity >= 3;
  var blah1 = DEBUG || self._options.verbosity >= 1;
  ...
  for (var xfx = 0; xfx < numXfs; xfx++) {
    ...
    if (!self.formattingInfo) continue;
    if (xf.is_style) continue;

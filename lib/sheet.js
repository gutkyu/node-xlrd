...
module.exports = function Sheet(workbook, sheetStreamReader, sheetName, sheetIndex){
    var self = this;
    ...
    self.read = function(){
        ...
        while(true){
            ...
            if (rc == comm.XL_NUMBER){
                ...
                putCell(rowx, colx, comm.XL_CELL_TEXT, book._sharedStrings[sstIndex], xfIndex);
                if (doSstRichText){
                ...
                if ( bv < comm.BIFF_FIRST_UNICODE)
                    strg = comm.unpackString(data, 6, book.encoding || book.deriveEncoding(), 2);
                else
                ...
                var rowx = data.readUInt16LE(0),
                    ...
                    strg = '';
                if ( bv < comm.BIFF_FIRST_UNICODE){
                    var unpack = comm.unpackStringUpdatePos(data, 6, book.encoding || book.deriveEncoding(), 2), 
                        pos = unpack[1];
					strg = unpack[0];
                    var nrt = data[pos];
                    pos += 1;
				...
				*/
                ...
                if ( boftype != 0x20) // embedded chart
                    console.log(util.format("*** Unexpected embedded BOF (0x%04x) at offset %d: version=0x%04x type=0x%04x" 
                        ,rc, shReader.cur - dataLen - 4, version, boftype));
                while (1){

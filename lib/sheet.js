...
var allowedEvents = ['row', 'done', 'error'];
...
module.exports = function Sheet(
  ...
  var self = this;
  var shReader = sheetStreamReader;
  ////
  // Name of sheet.
  var name = sheetName;
  ...
  if (book._options.raggedRows) self.putCell = putCellRagged;
  else self.putCell = putCellUnragged;
  ...
  //when eventMode == false, fallowing variables are used
  var _cellValues = [];
  var _cellTypes = [];
  var _cellXFIndexes = [];
  if (book._options.eventMode) {
    _cellValues = null;
    _cellTypes = null;
    _cellXFIndexes = null;
  }
  //when eventMode == true, fallowing variables are used
  var _rowCurIndex = -1;
  var _rowValues = null;
  var _rowRaws = null; // event mode 에서만 존재, 특히 cell type 이 date 일 경우 raw 값과 js object 로 변환한 결과가 상이할 때 근원값을 보관하기 위해 사용.
  var _rowTypes = null;
  var _rowXFIndexes = null;
  ...
  self.visibility = book._sheetVisibility[sheetIndex]; // from BOUNDSHEET record
  ...
  if (self.biffVersion >= 80) self.utterMaxRows = 65536;
  else self.utterMaxRows = 16384;
  ...
  function tidyDimensions() {
    if (book._options.verbosity >= 3)
      console.log(
        util.format('tidyDimensions: nrows=%d ncols=%d \n', _nRows, _nCols)
      );
    if (1 && self.mergedCells && self.mergedCells.length) {
      ...
      self.mergedCells.forEach(function (x) {
        var rlo = x[0],
          ...
          chi = x[3];
        if (
          !(0 <= rlo && rlo < rhi && rhi <= uMaxRows) ||
          !(0 <= clo && clo < chi && chi <= uMaxCols)
        )
          console.log(
            util.format(
              '*** WARNING: sheet #%d (%s), MERGEDCELLS bad range %s\n',
              self.index,
              self.name,
              crange
            )
    ...
    if (
      book._options.verbosity >= 1 &&
      (_nRows != self._dimnRows || _nCols != self._dimnCols)
    )
      console.log(
        util.format(
          'NOTE *** sheet %d (%s): DIMENSIONS R,C = %d,%d should be %d,%d\n',
          self.index,
          self.name,
          self._dimnRows,
          self._dimnCols,
    ...
    if (!book._options.raggedRows) {
      ...
      if (self._firstFullRowIndex == -2) ubound = _nRows;
  ...
  function putCellRagged(rowx, colx, ctype, value, xfIndex) {
    ...
    assert(
      0 <= colx && colx < self.utterMaxCols,
      '0 <= colx < self.utterMaxCols'
    );
    assert(
      0 <= rowx && rowx < self.utterMaxRows,
      '0 <= rowx < self.utterMaxRows'
    );
    if (book._options.eventMode && _rowCurIndex >= 0 && _rowCurIndex != rowx) {
      onChanged('row');
      _rowValues = null;
      _rowRaws = null;
      _rowTypes = null;
    ...
    if (_nRows < nr) {
      if (book._options.eventMode) {
        ...
        _rowValues = [];
        _rowRaws = [];
        if (fmtInf) _rowXFIndexes = [];
      } else {
        for (var i = _nRows; i < nr; i++) {
          _cellTypes[i] = [];
          _cellValues[i] = [];
    ...
    if (book._options.eventMode) {
      typesRow = _rowTypes;
      valuesRow = _rowValues;
      if (fmtInf) fmtRow = _rowXFIndexes;
    } else {
      typesRow = _cellTypes[rowx];
      valuesRow = _cellValues[rowx];
      if (fmtInf) fmtRow = _cellXFIndexes[rowx];
    }
    var ltr = typesRow.length;
    if (colx >= _nCols) _nCols = colx + 1;
    var numEmpty = colx - ltr;

    typesRow[colx] = ctype;
    if (book._options.eventMode) {
      valuesRow[colx] =
        ctype === comm.XL_CELL_DATE ? toDate(value, book.dateMode) : value;
      _rowRaws = value;
    } else {
      valuesRow[colx] = value;
    }
    if (fmtInf) fmtRow[colx] = xfIndex;

    if (numEmpty > 0) {
      for (i = ltr; i < colx; i++) {
        if (typesRow[i] === undefined) typesRow[i] = self.bt;
        if (valuesRow[i] === undefined) valuesRow[i] = '';
        if (fmtInf) if (fmtRow[i] === undefined) fmtRow[i] = self.bf;
      }
    }
    _rowCurIndex = rowx;
  }
    ...
    if (nr <= _nRows) {
      ...
      if (nextra > 0) {
        ...
        if (self.formattingInfo) {
          ...
          for (c = trow.length; c < _nCols; c++)
            _cellXFIndexes[rowx][c] = self.bf;
        }
  ...
  function putCellUnragged(rowx, colx, ctype, value, xfIndex) {
    ...
    var typesRow = book._options.eventMode ? _rowTypes : _cellTypes[rowx];
    var valuesRow = book._options.eventMode ? _rowValues : _cellValues[rowx];
    var fmtRow = null;
    if (self.formattingInfo)
      fmtRow = book._options.eventMode ? _rowXFIndexes : _cellXFIndexes[rowx];
    if (
      typesRow === null ||
      typesRow === undefined ||
      typesRow[colx] === undefined
    ) {
      extendCells(rowx, colx);
      typesRow = book._options.eventMode ? _rowTypes : _cellTypes[rowx];
      valuesRow = book._options.eventMode ? _rowValues : _cellValues[rowx];
    }
    typesRow[colx] = ctype;
    valuesRow[colx] =
      ctype === comm.XL_CELL_DATE ? toDate(value, book.dateMode) : value;
    ...
    if (self.formattingInfo) fmtRow[colx] = xfIndex;
  }
  ...
  function onChanged(event, param1) {
    if (event == 'row') {
      eventHandlers[event]({index: _rowCurIndex, values: _rowValues});
    } else if (event == 'done') {
      eventHandlers[event](_nRows);
    } else if (event == 'error') {
      eventHandlers[event](param1);
    }
  }
  ...
  if (!book._options.eventMode) {
    ...
    Object.defineProperty(self.row, 'count', {
      get: function () {
        return _nRows;
      },
    });
    ...
    self.row.getTypes = function (rowx, startColx, endColx) {
      ...
      if (endColx === undefined) return _cellTypes[rowx].slice(startColx);
    ...
    self.row.getRaws = function (rowx, startColx, endColx) {
      ...
      if (endColx === undefined) return _cellValues[rowx].slice(startColx);
  ...
  Object.defineProperty(self.column, 'count', {
    get: function () {
      return _nCols;
    },
  });
  ...
  self.read = function () {
    //global rc_stats
    var DEBUG = 0;
    var blah = DEBUG || book._options.verbosity >= 2;
    var blahRows = DEBUG || book._options.verbosity >= 4;
    var blahFormulas = 0 && blah;
    var r1c1 = 0;
    var XL_SHRFMLA_ETC_ETC = [
      comm.XL_SHRFMLA,
      comm.XL_ARRAY,
      comm.XL_TABLEOP,
      comm.XL_TABLEOP2,
    ...
    while (true) {
      ...
      if (rc == comm.XL_NUMBER) {
        ...
        var rowx = rd.u16; // offset 0
        var colx = rd.u16; // offset 2
        var xfIndex = rd.u16; // offset 4
        var d = rd.dbl; // offset 6
        // if xfIndex == 0:
        ...
        var rowx = rd.u16, // offset 0
          colx = rd.u16, // offset 2
          xfIndex = rd.u16, // offset 4
          sstIndex = rd.i32; // offset 6
        // print "LABELSST", rowx, colx, sstIndex, bk._sharedStrings[sstIndex]
        putCell(
          rowx,
          colx,
          comm.XL_CELL_TEXT,
          book._sharedStrings[sstIndex],
          xfIndex
        );
        if (doSstRichText) {
          var runList = book._richTextRunlistMap.get(sstIndex);
          if (runList) {
        ...
        if (bv < comm.BIFF_FIRST_UNICODE)
          strg = comm.unpackString(
            data,
            6,
            book.encoding || book.deriveEncoding(),
            2
          );
        else strg = unpackUnicode(data, 6, 2);
        ...
        var rowx = rd.u16, // offset 0
          ...
          strg = '';
        if (bv < comm.BIFF_FIRST_UNICODE) {
          var unpack = comm.unpackStringUpdatePos(
              data,
              6,
              book.encoding || book.deriveEncoding(),
              2
          ...
          var nrt = data[pos];
          pos += 1;
          while (pos < nrt) runList.append(data.getU8(pos++));
          assert(pos == data.length, 'pos == len(data)');
        } else {
          var unpack = comm.unpack_unicode_update_pos(data, 6, 2),
            pos = unpack[1];
          strg = unpack[0];
          var nrt = data.getU16L(0);
          pos += 2;
          while (pos < nrt) {
            runList.append(data.getU16L(pos));
            pos += 2;
          }
          assert(pos == data.length, 'pos == len(data)');
        }
        ...
        var mulrkRow = rd.u16; // offset 0
        var mulrkFirst = rd.u16; // offset 2
        var mulrkLast = rd.mov(data.length - 2).u16; // offset data.length - 2
        var pos = 4;
        for (colx = mulrkFirst; colx < mulrkLast + 1; colx++) {
          xfIndex = rd.mov(pos).u16;
        ...
        if (!fmtInf) continue;
        var rowx = rd.u16; // offset 0
        var bits1 = rd.mov(6).u16; // offset 6
        var bits2 = rd.mov(12).i32; // offset 12
        ...
        if (!r) {
          ...
          if (!r.has_default_xf_index) r.xfIndex = -1;
        }
        self.rowInfoMap[rowx] = r;
        if (0 && r.xfIndex > -1) {
          console.log(
            util.format('**ROW %d %d %d\n', self.index, rowx, r.xfIndex)
          );
        }
        if (blahRows) {
        ...
        comm.XL_FORMULA_OPCODES[0] == rc ||
        comm.XL_FORMULA_OPCODES[1] == rc ||
        comm.XL_FORMULA_OPCODES[2] == rc
      ) {
        // 06, 0206, 0406
        //todo 나중에 구현
        /*
        ...
        var colx = rd.u16; // offset 2
        var xfIndex = rd.u16; // offset 4
        var value = rd.u8; //offset 6
        var isErr = rd.u8; // offset 7
        ...
        self.defaultColWidth = rd.u16; // offset 0
        if (0) console.log('DEFCOLWIDTH', self.defaultColWidth);
      } else if (rc == comm.XL_STANDARDWIDTH) {
        if (dataLen != 2)
          console.log('*** ERROR *** STANDARDWIDTH', dataLen, repr(data));
        self.standardWidth = rd.u16; // offset 0
      ...
      } else if (rc == comm.XL_MULBLANK) {
        // 00BE
        if (!fmtInf) continue;
        var nitems = dataLen >> 1;
        var result = [];
        for (i = 0; i < nitems; i += 2) {
          result.push(rd.mov(i).u16);
        }
        var rowx = result[0],
          mul_first = result[1];
        ...
        var dimTuple = [];
        if (bv < 80) {
          dimTuple = [rd.mov(2).u16, rd.mov(6).u16];
        } else {
          dimTuple = [rd.mov(4).i32, rd.mov(10).u16];
        }
        _nRows = _nCols = 0;
        ...
        if (
          (bv == 21 || bv == 30 || bv == 40) &&
          book.xfList &&
          !book._xfEpilogueDone
        )
          book.xfEpilogue();
        if (blah)
          console.log(
            util.format(
              'sheet %d(%s) DIMENSIONS: ncols=%d nrows=%d',
              self.index,
              self.name,
              self._dimnCols,
              self._dimnRows
      ...
      } else if (comm.bofCodes.indexOf(rc) >= 0) {
        ////////// EMBEDDED BOF //////////
        var version = rd.u16, // offset 0
          boftype = rd.u16; // offset 2
        if (boftype != 0x20)
          // embedded chart
          console.log(
            util.format(
              ...
              rc,
              shReader.cur - dataLen - 4,
              version,
              boftype
            )
          );
        while (1) {
          var record = book.getRecordParts();
          var code = record[0];
          dataLen = record[1];
          data = record[2];
          rd = data.getLEReader();
          if (code == comm.XL_EOF) break;
        }
        if (DEBUG) console.log('---> found EOF');
      } else if (rc == comm.XL_COUNTRY) {
        book.handleCountry(data);
        ...
        var row1x = rd.u16; // offset 0
        var rownx = rd.u16; // offset 2
        var col1x = rd.u8; // offset 4
        var colnx = rd.u8; // offset 5
        var arrayFlags = rd.u8; // offset 6
        var tokslen = rd.mov(12).u16; // offset 12
        ...
        if (blah)
          console.log(
            util.format('MERGEDCELLS: %d ranges\n', Math.floor((pos - 2) / 8))
          );
        assert(
          pos == dataLen,
          util.format('MERGEDCELLS: pos=%d dataLen=%d', pos, dataLen)
        );
      } else if (rc == comm.XL_WINDOW2) {
        if (bv >= 80 && dataLen >= 14) {
          var options = rd.u16; // offset 0
          self.firstVisibleRowIndex = rd.u16; // offset 2
          self.firstVisibleColIndex = rd.u16; // offset 4
          self.gridlineColourIndex = rd.u16; // offset 6
          self.cachedPageBreakPreviewMagFactor = rd.mov(10).u16; // offset 10
          self.cachedNormalViewMagFactor = rd.u16; // offset 12
        } else {
          assert(bv >= 30, 'bv >= 30'); // BIFF3-7
          var options = rd.u16; // offset 0
          self.firstVisibleRowIndex = rd.u16; // offset 2
          self.firstVisibleColIndex = rd.u16; // offset 4
          self.gridlineColourRgb = [
            rd.u8, // offset 6
            rd.u8, // offset 7
            rd.u8, // offset 8
          ];
        ...
        var nitems = dataLen >> 1;
        var result = [];
        for (i = 0; i < nitems; i += 2) result.push(rd.mov(i).u16);
        var rowx = result[0],
          mul_first = result[1];
        var mulLast = result[result.length - 1];
        // print >> self.logfile, "MULBLANK", rowx, mul_first, mulLast, dataLen, nitems, mulLast + 4 - mul_first
    ...
    if (!eofFound)
      throw new Error(
        util.format('Sheet %d (%s) missing EOF record', self.index, self.name)
      );
    tidyDimensions();
    updateCookedMagFactors();
    if (book._options.eventMode && _rowCurIndex >= 0) {
      onChanged('row');
      _rowValues = null;
      _rowRaws = null;
      _rowTypes = null;
  ...
  self._executeCallbackMode = function () {
    return Promise.resolve()
      ...
      .catch(function (err) {
        if (existsEvent('error')) {
          onChanged('error', err);
          return Promise.resolve({isStop: true}); //다음 sheet pasring 종료
        } else throw err; //상위 전파
      });
    try {
      self.read();
    } catch (err) {
      if (existsEvent('error')) return onChanged('error', err);
      else throw err;
    }
    onChanged('done');
  };
  ...
  function updateCookedMagFactors() {
    ...
    var blah = DEBUG || book._options.verbosity > 0;
    if (self.showInPageBreakPreview) {
      if (!self.sclMagFactor)
        // no SCL record
        self.cookedPageBreakPreviewMagFactor = 100;
      // Yes, 100, not 60, NOT a typo
      else self.cookedPageBreakPreviewMagFactor = self.sclMagFactor;
        ...
        if (blah)
          console.log(
            util.format(
              ...
              self.index,
              self.cachedNormalViewMagFactor
            )
          );
      ...
      if (!self.sclMagFactor)
        // no SCL record
        self.cookedNormalViewMagFactor = 100;
      else self.cookedNormalViewMagFactor = self.sclMagFactor;
      ...
      if (zoom == 0)
        ...
        if (blah)
          console.log(
            util.format(
              'WARNING *** WINDOW2 rcd sheet %s: Bad cachedPageBreakPreviewMagFactor: %s',
              self.index,
              self.cachedPageBreakPreviewMagFactor
            )
          );
...
function unpackRK(data, start, end) {
  ...
  if (flags & 2) {
    ...
    var dbl = _dblBuf.getDblL(0); //type is double
    if (flags & 1) return dbl / 100.0;
    return dbl;
  }
}
...
function toDate(xldate, dateMode) {
  ...
  if (xldays == 0) return new Date(0, 0, 0, hour, minute, second);
  ...
  if (mp >= 10)
    return new Date(
      Math.floor(yreg / 1461) - 4715,
      mp - 10, //(mp - 9) -1; js Date's month is zeor-based (0 ~ 11)
      d,
      hour,
      minute,
      second
    ...
    return new Date(
      Math.floor(yreg / 1461) - 4716,
      mp + 2, //(mp +3) - 1; js Date's month is zeor-based (0 ~ 11)
      d,
      hour,
      minute,
      second

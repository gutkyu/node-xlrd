...
module.exports = function Sheet(book, position, name, index){
	...
	self.visibility = book._sheet_visibility[index]; // from BOUNDSHEET record
    ...
    function tidy_dimensions(){
        ...
        if (!self.ragged_rows){
            ...
            var ncols = _ncols,
				...
				s_fmt_info = self.formattingInfo;
            // for rowx in xrange(_nrows):
			var ubound = self._first_full_rowx;
            ...
            for(var rowx =0;rowx < ubound ; rowx++){
                ...
                if(nextra > 0){
					...
					for(var c =rlen;c < ncols-1; c++) s_cell_values[rowx][c]='';
                    ...
                    if (s_fmt_info) {
						...
						for(var c =rlen;c < ncols-1; c++) s_cell_xf_indexes[rowx][c]=self.bf;
					}
				}
			}
		}
	...
	function put_cell_ragged(rowx, colx, ctype, value, xfIndex){
        if (!ctype){
        ...
        var fmt_info = self.formattingInfo;
		...
		if (elf.nrows < nr){
			for(var i = elf.nrows ; i < nr ; i++ ){
				_cellTypes[i] = [];
				_cellValues[i] =[];
				if(fmt_info) self._cellXFIndexes[i] =[];
			}
			_nrows = nr;
		}
    ...
    function put_cell_unragged(rowx, colx, ctype, value, xfIndex){
		if(!ctype)
            // we have a number, so look up the cell type
            ctype = self._xf_index_to_xl_type_map[xfIndex];
	...
	*/
    ...
    self.cell.getXFIndex = function(rowx, colx){
        ...
        if((xfx=self.rowinfo_map[rowx].xfIndex)!== undefined){
            if (xfx > -1){
        ...
        if((xfx = self.colinfo_map[colx].xfIndex)!== undefined){
            if (xfx == -1) xfx = 15;
    ...
    self.read = function(bk){
        ...
        while(true){
            ...
            if (rc == comm.XL_NUMBER){
                ...
                self_put_cell(rowx, colx, null, d, xfIndex);
                ...
                self_put_cell(rowx, colx, comm.XL_CELL_TEXT, bk._sharedstrings[sstindex], xfIndex);
                if (do_sst_rich_text){
                ...
                self_put_cell(rowx, colx, comm.XL_CELL_TEXT, strg, xfIndex);
				...
				for(colx = mulrk_first; colx < mulrk_last+1; colx++){
                    ...
                    pos += 6;
                    self_put_cell(mulrk_row, colx, null, d, xfIndex);
				}
			...
			/*
                ...
                if ( result_str[6:8] == b"\xFF\xFF"){
                    ...
                    if ( first_byte == 0)
                        ...
                        self.put_cell(rowx, colx, comm.XL_CELL_TEXT, strg, xfIndex);
                        // if DEBUG: print "FORMULA strg %r" % strg
                    }else if( first_byte == 1){
                        // boolean formula result
                        value = BYTES_ORD(result_str[2]);
                        self_put_cell(rowx, colx, comm.XL_CELL_BOOLEAN, value, xfIndex);
                    }else if( first_byte == 2){
                        // Error in cell
                        value = BYTES_ORD(result_str[2]);
                        self_put_cell(rowx, colx, comm.XL_CELL_ERROR, value, xfIndex);
                    }else if( first_byte == 3){
                        // empty ... i.e. empty (zero-length) string, NOT an empty cell.
                        self_put_cell(rowx, colx, comm.XL_CELL_TEXT, "", xfIndex);
                    }else
			...
			*/
                ...
                var cellTyp = [comm.XL_CELL_BOOLEAN, comm.XL_CELL_ERROR][is_err];
                // if DEBUG: print "XL_BOOLERR", rowx, colx, xfIndex, value, is_err
                self_put_cell(rowx, colx, cellTyp, value, xfIndex);
			...
			*/
                ...
                self_put_cell(rowx, colx, comm.XL_CELL_BLANK, '', xfIndex);
			...
			*/
            ...
            //////// all of the following are for BIFF <= 4W
                ...
                if ( rc == comm.XL_FORMAT || rc == comm.XL_FORMAT2){
                    ...
                    cellTyp = (comm.XL_CELL_BOOLEAN, comm.XL_CELL_ERROR)[is_err];
                    // if DEBUG: print "XL_BOOLERR_B2", rowx, colx, cell_attr, value, is_err
                    self_put_cell(rowx, colx, cellTyp, value, self.fixed_BIFF2_xfindex(cell_attr, rowx, colx));
            ...
            if (!(10 <= zoom && zoom <=400))
                if (blah)
                    console.log(util.format(
                        ...
                        self.index, self.cached_normal_view_mag_factor
                        ));
            ...
            if (zoom == 0)
                ...
                if (blah)
                    console.log(util.format(
                        ...
                        ,self.index, self.cached_page_break_preview_mag_factor)
                        );

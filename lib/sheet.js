...
module.exports = function Sheet(book, position, name, index){
    ...
    function tidyDimensions(){
        ...
        if(self.verbosity >= 1 && (_nrows != self._dimnRows || _ncols != self._dimnCols))
            console.log(util.format("NOTE *** sheet %d (%s): DIMENSIONS R,C = %d,%d should be %d,%d\n",
                self.index,
                self.name,
                self._dimnRows,
                self._dimnCols,
	...
	*/
    ...
    self.read = function(bk){
        ...
        while(true){
			...
			*/
                ...
                if ( (bv == 21 || bv ==  30 || bv ==  40) && book.xfList && ! book._xfEpilogueDone)
                    book.xfEpilogue();
                if ( blah)
                    console.log(util.format("sheet %d(%s) DIMENSIONS: ncols=%d nrows=%d",
        ...
        if (!eofFound)
            throw new Error(util.format("Sheet %d (%s) missing EOF record",self.index, self.name));
        tidyDimensions();

...
module.exports = function Sheet(
  ...
  if (self.raggedRows) self.putCell = putCellRagged;
  ...
  function putCellUnragged(rowx, colx, ctype, value, xfIndex) {
    ...
    if (
      _cellTypes[rowx] === undefined ||
      _cellTypes[rowx][colx] === undefined
    ) {
      extendCells(rowx, colx);
    }
    _cellTypes[rowx][colx] = ctype;
    _cellValues[rowx][colx] = value;
    if (self.formattingInfo) _cellXFIndexes[rowx][colx] = xfIndex;
  }
...
var _nativeEndianIsLittleEndian =
  Uint8Array.from(Uint16Array.of(0x00, 0xff))[1] == 0xff;
...
function unpackRK(data, start, end) {
  ...
  if (flags & 2) {
    ...
    if (!_nativeEndianIsLittleEndian) buf = buf.reverse();
    var dbl = Float32Array.from(buf); //type is double
    var d = dbl[0];
    if (flags & 1) return d / 100.0;
    return d;
  }
}

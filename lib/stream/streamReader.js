...
module.exports = function StreamReader(stream, endian) {
  ...
  self.mov = function (offset) {
    ...
    if (mCur >= len)
      throw new Error(
        util.format(
          'StreamReader Error : offset %d is out of boundary %d',
          mCur,
          len
        )
      );
  ...
  Object.defineProperty(self, 'u8', {
    get: function () {
      if (cur >= len)
        throw new Error(
          util.format(
            'StreamReader Error : u8 current position %d is out of boundary %d',
            cur,
            len
          )
        );
  ...
  Object.defineProperty(self, 'u16', {
    get: function () {
      if (cur >= len - 1)
        throw new Error(
          util.format(
            'StreamReader Error : u16 current position %d is out of boundary %d',
            cur,
            len
          )
        );
  ...
  Object.defineProperty(self, 'i16', {
    get: function () {
      if (cur >= len - 1)
        throw new Error(
          util.format(
            'StreamReader Error : i16 position %d is out of boundary %d',
            cur,
            len
          )
        );
  ...
  Object.defineProperty(self, 'u32', {
    get: function () {
      if (cur >= len - 3)
        throw new Error(
          util.format(
            'StreamReader Error : u32 position %d is out of boundary %d',
            cur,
            len
          )
        );
  ...
  Object.defineProperty(self, 'i32', {
    get: function () {
      if (cur >= len - 3)
        throw new Error(
          util.format(
            'StreamReader Error : i32 position %d is out of boundary %d',
            cur,
            len
          )
        );
  ...
  self.str = function (encoding, readSize) {
    ...
    var ret = stream.slice(cur, cur + readSize).getStr(encoding);
    cur += readSize;
    return ret;
  };
  //readSize : byte count
  self.i32s = function (readSize) {
    if (readSize) {
      if (readSize & 0x03)
        throw new Error(
          util.format(
            'StreamReader Error : i32s readSize %d is not a multiple of 4',
            readSize
          )
        );
      if (cur > len - readSize)
        throw new Error(
          util.format(
            'StreamReader Error : i32s position %d is out of boundary %d',
            cur,
            len
          )
        );
    ...
    var i = cur;
    var bEnd =
      readSize === null || readSize === undefined ? len : cur + readSize;
  ...
  self.buf = function (readSize) {
    ...
    var ret = new Uint8Array(readSize);
  ...
  self.dump = function (start, end) {
    ...
    if (start && start >= end)
      throw new Error(
        util.format(
          'StreamReader Error : dump start %d is greater than end %d',
          start,
          end
        )
      );
    if (end && end > self.length)
      throw new Error(
        util.format(
          'StreamReader Error : dump end %d is out of boundary %d',
          end,
          len
        )
      );

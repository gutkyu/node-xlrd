...
exports.CompDoc = function (fd, totalSize){
	...
	function buildMSAT(){
		return 
		reader(76,512).then(function(buf){
	...
	*/
	...
	function buildDir(){
		return 
		getStream(self,wbuf, 512, sat, self.secSize, dirFirstSecSid,null,"directory",3)
		.then(function(buf){
			dirList = [];
...
function locateStream(compDoc, wbuf, base, sat, secSize, startSid, expectedStreamSize, qname, seenId){
	...
	if (!slices.length){
		...
		return 
		reader(startPos,startPos+expectedStreamSize)
		.then(function(buf){
			buf.hasData = true;

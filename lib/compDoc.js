...
var SIGNATURE = (exports.SIGNATURE = Uint8Array.of(
  0xd0,
  0xcf,
  0x11,
  0xe0,
...
//parameter dent is instance Of Uint8Array
function DirNode(DID, dent) {
  var self = this;
  // dent is the 128-byte directory entry
  self.DID = DID;
  var arrReader = dent.getLEReader();
  arrReader.mov(64);
  var cBufSize = arrReader.u16; //64
  self.etype = arrReader.u8; //66
  self.colour = arrReader.u8; //67
  self.leftDID = arrReader.i32; //68
  self.rightDID = arrReader.i32; //72
  self.rootDID = arrReader.i32; //76

  arrReader.mov(116);
  self.firstSID = arrReader.i32; //116
  self.totalSize = arrReader.i32; //120
  if (cBufSize == 0) self.name = '';
  //UNICODE_LITERAL('')
  else self.name = dent.getStr('utf16le', 0, cBufSize - 2); // unicode(dent[0:cBufSize-2], 'utf_16_le'); // omit the trailing U+0000
...
function CompDoc(fd, totalSize) {
  ...
  var sig = xlReader.buf(8);

  if (!sig.equals(SIGNATURE)) throw new Error('Not an OLE2 compound document');
  xlReader.mov(28);
  var endianMaker = xlReader.buf(2);
  if (!endianMaker.equals(Uint8Array.of(0xfe, 0xff)))
    throw new Error(
      'Expected "little-endian" marker [0xfe, 0xff], but found ' +
        endianMaker.toString()
    );
  xlReader.mov(24);
  ...
  /*
   * Interrogate the compound document's directory; return Uint8Array if found, otherwise
   * return null.
...
//return nodejs's Uint8Array object
function preloadStream(stream, sectorSequence, totalSize) {
  ...
  return Uint8Array.concat(secBufs, totalSize);
}

...
exports.CompDoc = function (fd, totalSize){
	...
	if(wbuf.toStr('hex',0,2) != 'feff')
		throw new Error('Expected "little-endian" marker, found ' + wbuf.toStr('hex',0,2));
	wbuf.pull(24,28);
	...
	*/
	self.locateNamedStream = function(qname){
		...
		if (d.totalSize > self.memDataLen)
			throw new  Error(util.format("%s stream length (%d bytes) > file data size (%d bytes)",
...
function getStream(compDoc, wbuf, base, sat, secSize, startSid, size, name, seenId){
	...
	if (size === null || size === undefined){
		...
		while( s >= 0){
		    ...
		    totalSz += secSize;
		    if(sat.length <= s) throw new Error(util.format("OLE2 stream %s: sector allocation table invalid entry (%d)",name, s));
		    s = sat[s];
		}
		...
		assert(s == EOCSID, 's == EOCSID');
		if (todo != 0)
			console.log(util.format("WARNING *** OLE2 stream %s: expected size %d, actual size %d\n",name, size, size - todo));
	}

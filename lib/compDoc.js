	...
	function buildMSAT(){
		return reader(76,512).then(function(buf){
			...
			if( msatxTotalSecs == 0  &&  [EOCSID, FREESID, 0].indexOf(msatxFirstSecSid) >=0){
				...
				seq = promise.resolve();
				var loop = function(){
					if(EOCSID == sid || FREESID == sid) return false;
	...
	self.locateNamedStream = function(qname){
		...
		if (d.totalSize >= minSizeStdStream){
			...
			*/
			...
			return getStream(self, likeBuffer(self.SSCS), 0, ssat, shortSecSize, d.firstSID,d.totalSize, qname + " (from SSCS)", null)
			.then(function(buf){
...
function locateStream(compDoc, wbuf, base, sat, secSize, startSid, expectedStreamSize, qname, seenId){
	...
	if (!slices.length){
		...
		return reader(startPos,startPos+expectedStreamSize)
		.then(function(buf){
			buf.hasData = true;
			return [buf, startPos, expectedStreamSize];
		})
	}else{
		slices.push([startPos, endPos]);

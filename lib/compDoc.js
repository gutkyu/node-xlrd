...
function likeBuffer(fd, size) {
  if (Buffer.isBuffer(fd)) {
    ...
    buf.getUInt8 = function (position) {
      if (position >= len)
        throw new Error(
          'likeBuffer Error : position %d of readUInt8 beyond buffer length %d',
          position,
          len
        );
    ...
    buf.getUInt16LE = function (position) {
      if (position >= len + 1)
        throw new Error(
          'likeBuffer Error : position %d of readUInt16LE beyond buffer length %d',
          position,
          len
        );
    ...
    buf.getInt16LE = function (position) {
      if (position >= len + 1)
        throw new Error(
          'likeBuffer Error : position %d of readInt16LE beyond buffer length %d',
          position,
          len
        );
    ...
    buf.getInt32LE = function (position) {
      if (position >= len + 3)
        throw new Error(
          'likeBuffer Error : position %d of readInt32LE beyond buffer length %d',
          position,
          len
        );
    ...
    buf.toStr = function (encoding, start, end) {
      ...
      if (start && start >= len)
        throw new Error(
          'likeBuffer Error : start %d of toString beyond buffer length %d',
          start,
          len
        );
      if (end && end > len)
        throw new Error(
          'likeBuffer Error : end %d of toString beyond buffer length %d',
          end,
          len
        );
    ...
    buf.toInt32LEs = function (start, end) {
      ...
      if (start && start >= len)
        throw new Error(
          'likeBuffer Error : start %d of toInt32LEs beyond buffer length %d',
          start,
          len
        );
      if (end && end > len)
        throw new Error(
          'likeBuffer Error : end %d of toInt32LEs beyond buffer length %d',
          end,
          len
        );

      var ret = [];
      for (var i = _start + start; i < _start + end; i += 4)
        ret.push(buf.readInt32LE(i));
      return ret;
    };
  ...
  function FileBuffer(fd, defaultBufferSize) {
    ...
    self.pull = function (start, end) {
      len = end - start;
      if (buf.length < len)
        buf = Buffer.concat([buf, Buffer.alloc(len - buf.length)], len);
      return fs.readSync(fd, buf, 0, len, start);
    };
    self.getUInt8 = function (position) {
      if (position >= len)
        throw new Error(
          'FileBuffer Error : position %d of readUInt8 beyond buffer length %d',
          position,
          len
        );
    ...
    self.getUInt16LE = function (position) {
      if (position >= len + 1)
        throw new Error(
          'FileBuffer Error : position %d of readUInt16LE beyond buffer length %d',
          position,
          len
        );
    ...
    self.getInt16LE = function (position) {
      if (position >= len + 1)
        throw new Error(
          'FileBuffer Error : position %d of readInt16LE beyond buffer length %d',
          position,
          len
        );
    ...
    self.getInt32LE = function (position) {
      if (position >= len + 3)
        throw new Error(
          'FileBuffer Error : position %d of readInt32LE beyond buffer length %d',
          position,
          len
        );
    ...
    self.toStr = function (encoding, start, end) {
      ...
      if (start && start >= len)
        throw new Error(
          util.format(
            'FileBuffer Error : start %d of toString beyond buffer length %d',
            start,
            len
          )
        );
      if (end && end > len)
        throw new Error(
          util.format(
            'FileBuffer Error : end %d of toString beyond buffer length %d',
            end,
            len
          )
        );
    ...
    self.toInt32LEs = function (start, end) {
      ...
      if (start && start >= len)
        throw new Error(
          'FileBuffer Error : start %d of toInt32LEs beyond buffer length %d',
          start,
          len
        );
      if (end && end > len)
        throw new Error(
          'FileBuffer Error : end %d of toInt32LEs beyond buffer length %d',
          end,
          len
        );
...
exports.CompDoc = function (fd, totalSize) {
  ...
  if (wbuf.toStr('hex', 0, 2) != 'feff')
    throw new Error(
      'Expected "little-endian" marker, found ' + wbuf.toStr('hex', 0, 2)
    );
  wbuf.pull(24, 28);

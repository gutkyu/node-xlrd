...
exports.CompDoc = function (fd, totalSize){
	...
	var ssz = null,
		sssz = null;
	var secSize =  self.secSize = 0;
	var shortSecSize = 0;
	var satTotalSecs = null;
	var dirFirstSecSid = null;
	var _unused = null;
	var minSizeStdStream = null;
	var ssatFirstSecSid = null;
	var ssatTotalSecs = null;
	var msatxFirstSecSid = null;
	var msatxTotalSecs = null;
		
	var memDataLen = null;
	var memDataSecs = null ,
		leftOver =  null;
	
	self.memDataSecs = null; // use for checking later
	self.memDataLen = null;
	var seen = self.seen = null;
	
	var nent = null;// number of SID entries in a sector
	var truncWarned = 0;
	
	function getHeader(){
    //read('h8:n16:u2:u2:h2:u2:u2:n10:u4:u4:u4:u4:u4:u4:u4:u4')
		return reader(0,8).then(function(buf){
			sig = buf.toStr('hex',0,8)
			if( sig != SIGNATURE) throw new Error('Not an OLE2 compound document');
			return reader(24,28);
		}).then(function(buf){
			revision = buf.getUInt16LE(0),
			version = buf.getUInt16LE(2);
			return reader(28, 30);
		}).then(function(buf){
			if(buf.toStr('hex',0,2) != 'feff')
			throw new Error('Expected "little-endian" marker, found %r' + buf.toStr('hex',0,2));
			return reader(30,34);
		}).then(function(buf){
			ssz = buf.getUInt16LE(0);
			sssz = buf.getUInt16LE(2);
			...
			secSize =  self.secSize =1 << ssz;
			shortSecSize = 1 << sssz;
			if (self.secSize != 512 || shortSecSize != 64)
				console.log("@@@@ secSize=%d short_sec_size=%d",self.secSize, shortSecSize);
			return reader(44,76);
		}).then(function(buf){
			satTotalSecs = buf.getInt32LE(0);
			dirFirstSecSid = buf.getInt32LE(4);
			_unused = buf.getInt32LE(8);
			minSizeStdStream = buf.getInt32LE(12);
			ssatFirstSecSid = buf.getInt32LE(16);
			ssatTotalSecs = buf.getInt32LE(20);
			msatxFirstSecSid = buf.getInt32LE(24);
			msatxTotalSecs = buf.getInt32LE(28);
			
			memDataLen = totalSize - 512;
			memDataSecs = Math.floor(memDataLen / secSize);
			...
			seen = self.seen = new Array(memDataSecs);
			...
			*/
			nent = Math.floor(secSize/4) ;// number of SID entries in a sector
			truncWarned = 0;
		});
    }
	//
	...
	var MSAT = null;
	var satSectorsReqd = null;
	var expectedMsatxSecs = null;
	var actualMsatxSecs = 0;
	function buildMSAT(){
		return reader(76,512).then(function(buf){
			MSAT = buf.toInt32LEs(0,436);
			satSectorsReqd = Math.floor((memDataSecs + nent - 1)/ nent);
			expectedMsatxSecs = Math.max(0, Math.floor((satSectorsReqd - 109 + nent - 2)/ (nent - 1)));
			actualMsatxSecs = 0;
			...
			if( msatxTotalSecs == 0  &&  [EOCSID, FREESID, 0].indexOf(msatxFirstSecSid) >=0){
				...
				seq = promise.resolve();
				var loop = function(){
					if(EOCSID == sid || FREESID == sid) return seq;
					return seq.then(function(){
						...
						if (sid >= memDataSecs){
							var msg = util.format("MSAT extension: accessing sector %d but only %d in file",sid, memDataSecs);
							//if DEBUG > 1:
						...
						return reader(offset,offset+secSize);
					}).then(function(buf){
						var buf = buf.toInt32LEs();
						sid = buf.pop();
	...
	function buildSAT(){
        ...
        var loop = function(){
            msidx++;
            if(msidx >= MSAT.length) return seq;
            return seq.then(function(){
				var msid = MSAT[msidx];
	...
	*/
	...
	var dirList = null;
	function buildDir(){
		return getStream(self,wbuf, 512, sat, self.secSize, dirFirstSecSid,null,"directory",3)
		.then(function(buf){
			dirList = [];
			for(var did =0, pos = 0; pos < buf.length; pos += 128){
				dirList.push(new DirNode(did++, buf.slice(pos,pos+128)));
			}
		...
		return promise.resolve()
                .then(function(){
                    if(sscsDir.firstSID < 0 || sscsDir.totalSize == 0){
                        ...
                        return getStream(self,wbuf,512, sat, secSize, sscsDir.firstSID,sscsDir.totalSize, "SSCS", 4)
                        .then(function(buf){
                            self.SSCS = buf;
                        });
                    }
			...
			if (sscsDir.totalSize > 0){
				...
				var loop = function(){
					...
					return seq.then(function(){
						...
						return reader(startPos,startPos+secSize);
					}).then(function(buf){
						ssat = ssat.concat(buf.toInt32LEs());
						sid = sat[sid];
						return loop();
					})
				};
	...
	*/
	self.getNamedStream = function (qname){
		...
		if( d === null ) return pms.then(function(){return null;});
		if( d.totalSize >= minSizeStdStream)
            return pms.then(function(){
                return getStream(self, wbuf, 512, sat, self.secSize, 
                    d.firstSID,d.totalSize, qname, d.DID+6);
            });
		else
            return pms.then(function(){
                return getStream(self, likeBuffer(self.SSCS), 0, ssat, shortSecSize, 
                    d.firstSID, d.totalSize, qname + " (from SSCS)", null);
            });
	}
	/*
	...
	if (size === null || size === undefined){
		...
		var loop = function(){
			...
			return seq.then(function(){
				...
				sectors.push(buf.toBuf());
				totalSz += secSize;
				if(sat.length <= s) 
					throw new Error(util.format("OLE2 stream %r: sector allocation table invalid entry (%d)",name, s));
				s = sat[s];
				return loop();
			});
		};
		...
		var loop = function(){
			...
			return seq.then(function(){
				...
				todo -= grab;
				return reader(startPos,startPos+grab);
			}).then(function(buf){
				sectors.push(buf.toBuf());
				totalSz += grab;
...
function locateStream(compDoc, wbuf, base, sat, secSize, startSid, expectedStreamSize, qname, seenId){
	...
	if (!slices.length){
		...
		return seq.then(function(buf){
			buf.hasData = true;
			return [reader, startPos, expectedStreamSize];
		})
	}else{
		slices.push([startPos, endPos]);

...
function DirNode(DID, dent){
	...
	self.left_DID = dent.readInt32LE(68);
	self.right_DID = dent.readInt32LE(72);
	self.root_DID = dent.readInt32LE(76);
	
	self.first_SID = dent.readInt32LE(116);
	self.tot_size = dent.readInt32LE(120);
...
exports.CompDoc = function (fd, totalSize){
	...
	var sec_size =  self.sec_size =1 << ssz;
    self.short_sec_size = 1 << sssz;
    if (self.sec_size != 512 || self.short_sec_size != 64)
            console.log("@@@@ sec_size=%d short_sec_size=%d",self.sec_size, self.short_sec_size);
    wbuf.pull(44,76);
    var SAT_tot_secs = wbuf.getInt32LE(0);
	self.dir_first_sec_sid = wbuf.getInt32LE(4);
	var _unused = wbuf.getInt32LE(8);
	self.min_size_std_stream = wbuf.getInt32LE(12);
	...
	var seen = self.seen = new Array(mem_data_secs);
	for(var i =0; i < mem_data_secs; i++ ) seen[i] = 0;
	/*
	if DEBUG:
	...
	*/
	...
	for(var msidx =0; msidx < MSAT.length; msidx++){
		...
		wbuf.pull(offset,offset+sec_size);
		self.SAT = self.SAT.concat(wbuf.toInt32LEs());
	}
	/*
	if DEBUG:
	...
	*/
	...
	// === build the directory ===
	//
	var dbytes = getStream(self,wbuf, 512, self.SAT, self.sec_size, self.dir_first_sec_sid,null,"directory",3);
	var dirlist = [];
	...
	*/
	...
	// === get the SSCS ===
	//
	var sscs_dir = self.dirlist[0];
	assert(sscs_dir.etype == 5 ,'sscs_dir.etype == 5 '); // root entry
	if(sscs_dir.first_SID < 0 || sscs_dir.tot_size == 0){
		self.SSCS = new Buffer(0);
	}else{
		self.SSCS = getStream(self,wbuf,512, self.SAT, sec_size, sscs_dir.first_SID,sscs_dir.tot_size, "SSCS", 4);
	}
	...
	if (sscs_dir.tot_size > 0){
		...
		while( sid >= 0 && nsecs > 0){
			...
			self.SSAT = self.SSAT.concat(wbuf.toInt32LEs());
	...
	*/
	self.get_named_stream = function (qname){
		var d = dirSearch(self,qname.split("/"));
		if( d === null ) return null;
		if( d.tot_size >= self.min_size_std_stream)
			return getStream(self, wbuf, 512, self.SAT, self.sec_size, d.first_SID,d.tot_size, qname, d.DID+6);
		else
			return getStream(self, likeBuffer(self.SSCS), 0, self.SSAT, self.short_sec_size, d.first_SID,
	...
	* @param qname Name of the desired stream e.g. u'Workbook'. Should be in Unicode or convertible thereto.
	*/
	self.locate_named_stream = function(qname){
		var d = dirSearch(self,qname.split("/"));
		...
		if (d.tot_size >= self.min_size_std_stream){
			...
			*/
			return locateStream(self, wbuf, 512, self.SAT, self.sec_size, d.first_SID, 
				d.tot_size, qname, d.DID+6);
		}else{
			return [
				getStream(self, likeBuffer(self.SSCS), 0, self.SSAT, self.short_sec_size, d.first_SID,d.tot_size, qname + " (from SSCS)", null),
				0,
				d.tot_size
			];
		}

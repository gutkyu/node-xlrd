...
function Workbook(interObj){
	...
	Object.defineProperty(self, 'sheets', {get:function(){
		for (sheetx =0; sheetx<_nsheets;sheetx)
            if (! _sheetList[sheetx])
                getSheet(sheetx);
        return _sheetList;
	}});
	
	Object.defineProperty(self, 'sheetCount', {get:function(){return _nsheets;}});

    //#
    // @param sheetx Sheet index in range(nsheets)
    // @return An object of the Sheet class
    self.sheetByIndex=function(sheetx){
        return _sheetList[sheetx] || getSheet(sheetx);
	}
    //#
    // @param sheet_name Name of sheet required
    // @return An object of the Sheet class
    self.sheetByName=function(sheetName){
        
        var sheetx = _sheetNames.indexOf(sheetName);
		if(sheetx<0) throw new Error(util.format('No sheet named <%s>' , sheetName));
        return self.sheetByIndex(sheetx);
	}
    //#
    // @return A list of the names of all the worksheets in the workbook file.
    // This information is available even when no sheets have yet been loaded.
	Object.defineProperty(self, 'sheetNames', {get:function(){return _sheetNames;}});
    ...
    self.sheetLoaded=function(sheetId){
        ...
        if (!(sheetId instanceof int)){
			sheetId = _sheetNames.indexOf(sheet_name_or_index);
			if(sheetx<0) throw new Error(util.format('No sheet named <%s>' , sheetId));
		}
        return bool(_sheetList[sheetId]);
	}
    //#

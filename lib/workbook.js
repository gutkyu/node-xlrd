...
var encodingFromCodePage = require('./utils/StringEncoding')
...
function Workbook(protectedAPI, fd, options) {
  ...
  function deriveEncoding() {
    if (self.encodingOverride) self.encoding = self.encodingOverride;
      ...
      if (Object.keys(encodingFromCodePage).indexOf(codePage.toString()) >= 0)
        encoding = encodingFromCodePage[codePage];

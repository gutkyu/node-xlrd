...
var compDoc = require('./compDoc');
var Sheet = require('./sheet');
var formatting = require('./formatting');
var stream = require('./stream');
var StreamReader = stream.StreamReader;
var strEncoding = require('./utils/StringEncoding');

var USE_FANCY_CD = 1;
var _EOF = 0x0f00baaa; // not a 16-bit number
var supportedVers = [80, 70, 50, 45, 40, 30, 21, 20];
var allowedEvents = ['done', 'error'];

//openWorkbook(filename, xlType, function(err, workbook))
exports.openWorkbook = function (fd, options) {
  var protectedAPI = {};
  var bk = new Workbook(protectedAPI, fd, options);
  if (!options.eventMode) protectedAPI.init();
  return bk;
};

function Workbook(protectedAPI, fd, options) {
  var self = this;
  ...
  self.on = function (event, cb) {
    ...
    if (typeof cb != 'function')
      throw new Error(
        'unsupported type : event "' + event + '" callback is not a function'
      );
    eventHandlers[event] = cb;
    return self;
  };

  function onChanged(event, param1) {
    var evtHnd = eventHandlers[event];
    if (event == 'done') {
      if (evtHnd) evtHnd();
    } else if (event == 'error') {
      if (evtHnd) evtHnd(param1);
    } else {
      console.log('workbook event "' + event + '" is not supported');
    }
  }

  function existsEvent(event) {
    return eventHandlers[event] !== undefined && eventHandlers[event] !== null;
  }

  // if eventMode == true, available
  if (options.eventMode) {
    self.work = function (cb) {
      if (cb === null || cb === undefined)
        throw new Error('Workbook.work callback should be defined.');
      if (!existsEvent('error'))
        throw new Error('Workbook event "error" handler should be defined.');
      ...
      Promise.resolve(init())
        ...
        .then(function () {
          //순차적으로 sheet parsing.
          //_executeCallbackMode()함수가 parsing 중 발생한 오류를 sheet error event로 처리했으면 result.isStop = true 반환, 더 이상 다음 sheet를 처리하지 않고 중단
          //if parsing row raise error, then just stop for-each and do not parsing
          var count = 0;
          var callSheetCallback = function (result) {
            if (result && result.isStop) return Promise.resolve();
            if (count < _sheetList.length) {
              var sh = _sheetList[count++];
              return sh._executeCallbackMode().then(callSheetCallback);
            } else {
              return Promise.resolve();
            }
          };
          return callSheetCallback({isStop: false});
        })
        .then(function () {
          onChanged('done');
        })
      ...
      var errSh = null;
      if (
        _sheetList.every(function (sh) {
          errSh = sh;
          return sh._executeCallbackMode();
        })
      ) {
        onChanged('done');
      } else {
        onChanged('error', new Error('sheet "' + errSh.name + '" error.'));
      }*/
    };
  }
  ...
  Object.defineProperty(self, 'sheets', {
    get: function () {
      for (var sheetIdx = 0; sheetIdx < _nsheets; sheetIdx++) {
        if (!_sheetList[sheetIdx]) getSheet(sheetIdx);
      }
      return _sheetList;
    },
  });
  ...
  self.sheet = {};
  Object.defineProperty(self.sheet, 'count', {
    get: function () {
      return _nsheets;
    },
  });
  ...
  self.sheet.loaded = function (sheetId) {
    if (!(typeof sheetId == 'number')) {
      ...
      if (sheetId < 0)
        throw new Error(util.format('No sheet named <%s>', sheetId));
    }
  ...
  var _fd = null;
  var _base = 0; // a base position of _stream
  var _stream = null;
  var _bkReader = null;

  if (protectedAPI) {
    protectedAPI.init = init;
    if (options.eventMode) {
      protectedAPI.onChanged = onChanged;
    }
  }
  ...
  function loadBiffN(fd) {
    ...
    if (bsig.equals(compDoc.SIGNATURE)) {
      var cd = compDoc.create(_fd, streamLength);
      if (USE_FANCY_CD) {
        if (
          !['Workbook', 'Book'].some(function (qname) {
            var ret = cd.locateNamedStream(qname);
            _stream = ret[0];
            _base = ret[1];
            streamLength = ret[2];
            return _stream && _stream.length ? true : false;
          })
        )
        ...
        if (
          !['Workbook', 'Book'].some(function (qname) {
            _stream = cd.getNamedStream(qname);
      ...
      _stream = stream.create(fd, null);
    }
    _bkReader = new StreamReader(_stream, 'little');
    _bkReader.mov(_base);
  }

  function initialiseFormatInfo() {
  ...
  function initSheetList() {
    _sheetList[_sheetNames.length - 1] = null;
  }
  ...
  function getSheetListLength() {
    return _sheetList.length;
  }

  //return UInt16
  var next2Bytes = function () {
    var remain = _stream.length - _bkReader.cur;
    if (remain < 2) {
      _bkReader.null(2);
      return _EOF;
    }
    return _bkReader.u16;
  };

  function getRecordPartsConditional(reqdRecord) {
    var code = _bkReader.u16;
    if (code != reqdRecord) {
      _bkReader.mov(_bkReader.cur - 2); //move back
      return [null, 0, ''];
    }
    var length = _bkReader.u16;
    var data = _bkReader.buf(length);
  ...
  function parseGlobals() {
    ...
    while (true) {
      var record = self.getRecordParts(_bkReader);
      var rc = record[0],
        length = record[1],
        data = record[2];
      ...
      switch (rc) {
        case comm.XL_SST:
          ...
          break;
        case comm.XL_FONT:
        case comm.XL_FONT_B3B4: //workbook.handle_font(data);
          break;
        ...
        case comm.XL_COUNTRY:
          self.handleCountry(data);
          break;
        case comm.XL_EXTERNNAME: //workbook.handle_externname(data);
          break;
        case comm.XL_EXTERNSHEET: //workbook.handle_externsheet(data);
          break;
        case comm.XL_FILEPASS: //workbook.handle_filepass(data);
          break;
        case comm.XL_WRITEACCESS:
          handleWriteAccess(data);
          break;
        case comm.XL_SHEETSOFFSET: //workbook.handle_sheetsoffset(data);
          break;
        case comm.XL_SHEETHDR:
          handleSheetHdr(data);
          break;
        case comm.XL_SUPBOOK: //workbook.handle_supbook(data);
          break;
        case comm.XL_NAME: //workbook.handle_name(data);
          break;
        case comm.XL_PALETTE: //workbook.handle_palette(data);
          break;
        case comm.XL_STYLE: //workbook.handle_style(data);
          break;
        case comm.XL_EOF:
          self.xfEpilogue();
          ...
          if (self.biffVersion == 45)
            ...
            console.log('global EOF: position', _bkReader.cur);
          // if DEBUG:
          //     pos = self._position - 4
          //     print repr(self.mem[pos:pos+40])
          return;
        default:
          if (rc & (0xff == 9) && options.verbosity) {
            console.log(
              util.format('*** Unexpected BOF at posn %d: %d len=%d data=%s'),
              _bkReader.cur - length - 4,
              rc,
              length,
              data
            );
  ...
  function read(length) {
    /*
		...
		*/
    var remain = _stream.length - _bkReader.cur;
    return _bkReader.buf(remain < length ? remain : length);
  }

  self.getRecordParts = function (reader) {
    var code = reader.u16;
    var length = reader.u16;
    return [code, length, reader.buf(length)]; //code, length, data
  };
  ...
  function getSheet(sheetIndex, updatePos) {
    ...
    if (updatePos) _bkReader.mov(_shAbsPosList[sheetIndex]);
    ...
    var sheetReader = new StreamReader(_stream, 'little');
    sheetReader.mov(_bkReader.cur);
    var sh = new Sheet(self, sheetReader, _sheetNames[sheetIndex], sheetIndex);
    if (!options.eventMode) sh.read();
  ...
  function fakeGlobalsGetSheet() {
    // for BIFF 4.0 and earlier
    formatting.initialiseWorkbook(self);
  ...
  function handleBoundSheet(data) {
    ...
    deriveEncoding();
    if (DEBUG) console.log(util.format('BOUNDSHEET: bv=%d data %s', bv, data));
    var sheetName = '';
    var visibility = 0;
    var sheetType = 0;
    var absPos = 0;
    if (bv == 45) {
      ...
      if (!_shAbsPosList.length) absPos = self._sheetsoffset + _base;
      // Note (a) this won't be used
      // (b) it's the position of the SHEETHDR record
      // (c) add 11 to get to the worksheet BOF record
      else absPos = -1; // unknown
    } else {
      var rd = data.getLEReader();
      var offset = rd.i32; //offset 0
      var visibility = rd.u8; // offset 4
      var sheetType = rd.u8; // offset 5
      absPos = offset + _base; // because global BOF is always at posn 0 in the stream
      if (bv < comm.BIFF_FIRST_UNICODE)
        sheetName = comm.unpackString(data, 6, self.encoding, 1);
      else sheetName = comm.unpackUnicode(data, 6, 1);
    }
    if (DEBUG || options.verbosity >= 2)
      console.log(
        util.format(
          'BOUNDSHEET: index=%d visibility=%s sheetName=%s absPos=%d sheetType=0x%02x',
          self._allSheetsCount,
          visibility,
          sheetName,
          absPos,
          sheetType
    ...
    if (sheetType != comm.XL_BOUNDSHEET_WORKSHEET) {
      ...
      if (DEBUG || options.verbosity >= 1)
        console.log(
          util.format(
            'NOTE *** Ignoring non-worksheet data named %s (type 0x%02x = %s)',
            sheetName,
            sheetType,
            descr
          )
  ...
  function getBOF(rqdStream) {
    // DEBUG = 1
    // if DEBUG: print >> self.logfile, "getbof(): position", self._position
    //if DEBUG: print("reqd: 0x%04x" % rqd_stream, file=self.logfile)
    var savPos = _bkReader.cur;
    var opcode = next2Bytes();
    if (opcode == _EOF) bofError('Expected BOF record; met end of file');
    if (comm.bofCodes.indexOf(opcode) < 0) {
      _bkReader.mov(savPos);
      bofError('Expected BOF record; found ' + _bkReader.buf(8).toString());
    }
    var length = next2Bytes();
    if (length == _EOF) bofError('Incomplete BOF record[1]; met end of file');
    if (!(4 <= length && length <= 20)) {
      bofError(
        util.format(
          'Invalid length (%d) for BOF record type 0x%s',
          length,
          opcode.toString()
        )
      );
    }
    var padding = new Uint8Array(Math.max(0, comm.bofLen[opcode] - length));
    padding.fill(0);
    var data = read(length); //Type Buffer
    //if DEBUG: fprintf(self.logfile, "\ngetbof(): data=%r\n", data);
    //if DEBUG: fprintf(self.logfile, "\ngetbof(): data=%r\n", data);
    if (data.length < length)
      bofError('Incomplete BOF record[2]; met end of file');
    data = Uint8Array.concat([data, padding]);
    var rd = data.getLEReader();
    var version1 = opcode >> 8;
    var version2 = rd.u16; // offset 0
    var streamType = rd.u16; // offset 2
    /*if DEBUG:
    ...
    var version = 0,
      ...
      year = 0;
    if (version1 == 0x08) {
      build = rd.u16; // offset 4
      year = rd.u16; // offset 6
    ...
    if (version == 40 && streamType == comm.XL_WORKBOOK_GLOBALS_4W)
      version = 45; // i.e. 4W
    ...
    /*if DEBUG or self.verbosity >= 2:
		...
		*/
    var gotGlobals =
      streamType == comm.XL_WORKBOOK_GLOBALS ||
      (version == 45 && streamType == comm.XL_WORKBOOK_GLOBALS_4W);
    if (
      (rqdStream == comm.XL_WORKBOOK_GLOBALS && gotGlobals) ||
      streamType == rqdStream
    )
      return version;
    if (version < 50 && streamType == comm.XL_WORKSHEET) return version;
    if (version >= 50 && streamType == 0x0100)
      bofError('Workspace file -- no spreadsheet data');
    bofError(
      util.format(
        'BOF not workbook/worksheet: op=%d vers=%d strm=%d build=%d year=%d -> BIFF%d',
        opcode,
        version2,
        streamType,
        build,
  ...
  function deriveEncoding() {
    if (self.encodingOverride) self.encoding = self.encodingOverride;
      ...
      if (
        Object.keys(strEncoding.encodingFromCodePage).indexOf(
          codePage.toString()
        ) >= 0
      )
        encoding = strEncoding.encodingFromCodePage[codePage];
      else if (300 <= codePage && codePage <= 1999) encoding = 'cp' + codePage;
      else encoding = 'unknown_codepage_' + codePage;
    ...
    if (self.codePage != 1200)
      if (!strEncoding.isEncoding(self.encoding))
        // utf_16_le
        // todo 지원하지 않을 경우 강제로 적용할 encoding 옵션
        throw new Error(
          'codePage ' + self.codePage,
          ' -> encoding -> ' + self.encoding + ' : not supported.'
        );
    /*
    ...
    if (userNameIsRaw) {
      ...
      var str = comm.decode(userName, 0, userName.length, self.encoding);
      var str = str.trimEnd();
  ...
  function handleCodePage(data) {
    // DEBUG = 0
    var codePage = data.getU16L(0);
    self.codePage = codePage;
    deriveEncoding();
  }
  self.handleCountry = function (data) {
    var countries = [data.getU16L(0), data.getU16L(2)];
    if (options.verbosity)
      console.log('Countries:', countries[0], ',', countries[1]);
    // Note: in BIFF7 and earlier, country record was put (redundantly?) in each worksheet.
    assert(
      (self.countries[0] == 0 && self.countries[1] == 0) ||
        (self.countries[0] == countries[0] &&
          self.countries[1] == countries[1]),
      'self.countries == [0, 0] or self.countries == countries'
    );
    self.countries = countries;
  };
  function handleDateMode(data) {
    var dateMode = data.getU16L(0);
    /*
  ...
  function handleSheetHdr(data) {
    ...
    // DEBUG = 1
    deriveEncoding();
    var sheetLen = data.getI32L(0); //offset 0
    var sheetName = comm.encode(data, 4, self.encoding, 1);
    var sheetNo = _sheetHdrCount;
    assert(
      sheetName == _sheetNames[sheetNo],
      'sheetName == _sheetNames[sheetno]'
    );
    _sheetHdrCount += 1;
    var bofPos = _bkReader.cur;
    ...
    console.log(
      util.format(
        'SHEETHDR %d at posn %d: len=%d name=%s\n',
        sheetNo,
        posn,
        sheetLen,
        sheetName
  ...
  function handleSST(data) {
    ...
    var nbt = data.length;
    var strList = [data];
    var uniqueStrings = data.getI32L(4); //offset 4
    //if DEBUG  or self.verbosity >= 2:
    ...
    if (self.formattingInfo) self._richTextRunlistMap = rtRunList;
    //if DEBUG:
    //	t1 = time.time()
  ...
  function init() {
    loadBiffN(fd);
    var biffVer = getBOF(comm.XL_WORKBOOK_GLOBALS);
    if (!biffVer) {
    ...
    if (biffVer <= 40) {
      ...
      if (options.onDemand) {
        ...
        self.onDemand = options.onDemand = false;
      }
      fakeGlobalsGetSheet();
    } else if (biffVer == 45) {
      parseGlobals();
      ...
      parseGlobals();
      initSheetList();
      if (!options.onDemand) getSheets();
      setSheetCount(getSheetListLength());
      if (biffVer == 45 && self.sheetCount > 1) {
        console.log(
...
function parseSST(dataTable, stringLength) {
  ...
  for (var _unused_i = 0; _unused_i < stringLength; _unused_i++) {
    ...
    if (options & 0x08) {
      // richtext
      rtCount = data.getU16L(pos);
      pos += 2;
    }
    if (options & 0x04) {
      // phonetic
      phoSize = data.getI32L(pos);
      pos += 4;
    }
    var accStr = '';
    var charsGot = 0;
    while (true) {
      ...
      if (options & 0x01) {
        ...
        try {
          accStr += rawStr.getStr('utf16le', 0, rawStr.length);
        } catch (e) {
        ...
        accStr += rawStr.getStr('latin1', 0, rawStr.length);
        pos += charsAvail;
      }
      charsGot += charsAvail;
    ...
    if (rtCount) {
      ...
      for (var runindex = 0; runindex < rtCount; runindex++) {
        ...
        runs.push(data.getU16L(pos));
        runs.push(data.getU16L(pos + 2));
        pos += 4;
      }

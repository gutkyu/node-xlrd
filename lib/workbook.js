...
function Workbook(interObj){
	...
	function handleBoundSheet(data){
        ...
        if(DEBUG || self.verbosity >= 2)
            console.log(util.format(
                "BOUNDSHEET: inx=%d vis=%r sheetName=%r absPos=%d sheetType=0x%02x",
	...
	function deriveEncoding(){
        ...
        if (userNameIsRaw){
            ...
            var strg = comm.encode(userName, 0, self.encoding, 1)
            var strg = strg.rtrim();
            // if DEBUG:
            //     print "CODEPAGE: user name decoded from %r to %r" % (self.userName, strg)
            self.userName = strg;
            userNameIsRaw = false;

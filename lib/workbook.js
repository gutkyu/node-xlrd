...
function Workbook(interObj){
		...
		if(bsig== compDoc.SIGNATURE){
            ...
            if(USE_FANCY_CD){
                if(!['Workbook', 'Book'].some(function(qname){
					...
					_base = ret[1];
					streamLength = ret[2];
                    return _stream && (_stream.length || _stream.hasData) ? true: false;
	...
	var next2Bytes = function(){
		...
		var remain = _stream.length - pos;
		if(remain < 2) { self._position += remain; return _EOF;}
		self._position += 2;
		return _stream.readUInt16LE(pos);
	};

	function getRecordPartsConditional(reqd_record){
        var pos = self._position;
        var code = _stream.readUInt16LE(pos), 
            length = _stream.readUInt16LE(pos+2);
        if (code != reqd_record)
            return [null, 0, ''];
        pos += 4;
        var data = _stream.slice(pos,pos+length);
	...
	function read(pos, length){
		...
		*/
		...
		self._position += len;
		return _stream.slice(pos,pos+len);
	}

	self.getRecordParts = function(){
		var pos = self._position;
		var code = _stream.readUInt16LE(pos) ;
		var length = _stream.readUInt16LE(pos + 2);
        pos += 4;
        var data = _stream.slice(pos,pos+length);
        self._position = pos + length;
        return [code, length, data];

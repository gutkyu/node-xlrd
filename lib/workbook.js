...
function Workbook(interObj){
	...
	var next2Bytes = function(){
		...
		if(remain < 2) { _bkReader.null(2); return _EOF;}
			...
			switch(rc){
				...
				default:
					if( rc & 0xff == 9 && self.verbosity){
						console.log(util.format("*** Unexpected BOF at posn %d: %d len=%d data=%r"), _bkReader.cur - length - 4, rc, length, data);
	...
	function handleBoundSheet(data){
        ...
        if (bv == 45){ // BIFF4W
            ...
            if (!_shAbsPosList.length)
                absPos = self._sheetsoffset + _base;
	...
	function getBOF(rqdStream){
		...
		*/
		...
		if ((rqdStream == comm.XL_WORKBOOK_GLOBALS && gotGlobals) || streamType == rqdStream) return version;

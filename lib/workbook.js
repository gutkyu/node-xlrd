...
function Workbook(interObj) {
      ...
      if (USE_FANCY_CD) {
        if (
          !['Workbook', 'Book'].some(function (qname) {
            ...
            return self.mem && (self.mem.length || self.mem.hasData)
              ? true
              : false;
          })
        )
  ...
  function getRecordPartsConditional(reqd_record) {
    ...
    if (code != reqd_record) return [null, 0, ''];
    pos += 4;
  ...
  function handleBoundSheet(data) {
    ...
    if (bv == 45) {
      ...
      if (!_shAbsPosList.length) absPos = self._sheetsoffset + self.base;
  ...
  function getBOF(rqd_stream) {
    ...
    if (comm.bofCodes.indexOf(opcode) < 0) {
      ...
      bofError('Expected BOF record; found %s', buf.toString('hex'));
    }
    ...
    */
    ...
    if (
      (rqd_stream == comm.XL_WORKBOOK_GLOBALS && gotGlobals) ||
      streamType == rqd_stream
    )
      return version;
  ...
  function deriveEncoding() {
    ...
    if(!comm.isEncoding(self.encoding))
      throw new Error('codePage ' + self.codePage, ' -> encoding -> ' + self.encoding +' : not supported.');
    /*

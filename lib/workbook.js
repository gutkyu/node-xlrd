...
exports.openWorkbook = function(fd, options, callback){
	...
	var biffVer = bk.getBOF(comm.XL_WORKBOOK_GLOBALS);
	if(!biffVer){
	...
	if(biffVer <= 40){
		...
		bk.parse_globals();
		bk.sheetList = bk.sheetNames.slice(0);
		if(!opts.onDemand) bk.getSheets();
		bk.nsheets = bk.sheetList.length;
		if(biffVer == 45 && bk.nsheets >1){
			console.log('*** WARNING: Excel 4.0 workbook (.XLW) file contains %d worksheets.',bk.nsheets);
			console.log('*** Book-level data will be that of the last worksheet.\n');
		}
	}
	bk.loadBiffN = null;
	bk.parse_globals = null;
	return  bk;
...
function Workbook(){
    ...
    self.sheets = function(){
        for (sheetx =0; sheetx<self.nsheets;sheetx)
            if (! self.sheetList[sheetx])
    ...
    self.sheetByIndex=function(sheetx){
        return self.sheetList[sheetx] || self.get_sheet(sheetx);
	}
    //#
    // @param sheet_name Name of sheet required
    // @return An object of the Sheet class
    self.sheetByName=function(sheet_name){
        
        var sheetx = self._sheet_names.indexOf(sheet_name);
		if(sheetx<0) throw new Error(util.format('No sheet named <%s>' , sheet_name));
        return self.sheetByIndex(sheetx);
	}
    //#
    // @return A list of the names of all the worksheets in the workbook file.
    // This information is available even when no sheets have yet been loaded.
    self.sheetNames=function(){
        return self._sheet_names;
	}
    //#
    // @param sheet_name_or_index Name or index of sheet enquired upon
    // @return true if sheet is loaded, false otherwise
    self.sheetLoaded=function(sheetId){
		
        if (!(sheetId instanceof int)){
			sheetId = self._sheet_names.indexOf(sheet_name_or_index);
			if(sheetx<0) throw new Error(util.format('No sheet named <%s>' , sheetId));
		}
        return bool(self.sheetList[sheetId]);
	}
    //#
    // @param sheet_name_or_index Name or index of sheet to be unloaded.
    self.unloadSheet=function(sheetId){
		if (!(sheetId instanceof int)){
			sheetId = self._sheet_names.indexOf(sheet_name_or_index);
			if(sheetx<0) throw new Error(util.format('No sheet named <%s>' , sheetId));
		}
        self.sheetList[sheetx] = null;
    }
	
	self.sheetNames = [];//_sheet_names
	self.sheetList = [];//_sheet_list
	self._position = 0;
		...
		if(bsig== compDoc.SIGNATURE){
            ...
            if(USE_FANCY_CD){
                if(!['Workbook', 'Book'].some(function(qname){
					var ret = cd.locate_named_stream(qname);
					self.mem = ret[0];
					self.base = ret[1];
	...
	function getSheet(sh_number, update_pos){
		...
		_unused_biff_version = self.getBOF(comm.XL_WORKSHEET)
		...
		var sh = new Sheet(self, self._position,self.sheetNames[sh_number],sh_number);
		sh.read(self);
		self.sheetList[sh_number] = sh;
		return sh;
	}
	...
	self.getSheets = function(){
		...
		var sheetno = 0;
		self.sheetNames.forEach(function(x){
            if (DEBUG) console.log("GET_SHEETS: sheetno =", sheetno, self.sheetNames, _sh_abs_posn);
			getSheet(sheetno++);
		});
	}
	self.fake_globals_get_sheet= function(){// for BIFF 4.0 and earlier
		formatting.initialise_book(self);
		var fake_sheet_name = 'Sheet 1';
		self.sheetNames = [fake_sheet_name];
		_sh_abs_posn = [0];
		self._sheet_visibility = [0]; // one sheet, visible
		self.sheetList.push(null); // get_sheet updates _sheet_list but needs a None beforehand
		self.getSheets();
	}
	...
	function handle_sheethdr(data){
        ...
        assert(sheet_name == self.sheetNames[sheetno],'sheet_name == self.sheetNames[sheetno]');
        _sheethdr_count += 1;
...
function parseSST(datatab, nstrings){
    ...
    var richtext_runs = {};
    var latin1 = "latin1";
    for(_unused_i =0;_unused_i < nstrings; _unused_i++){

...
function Workbook(interObj){
			...
			switch(rc){
				...
				default:
					if( rc & 0xff == 9 && self.verbosity){
						console.log(util.format("*** Unexpected BOF at posn %d: %d len=%d data=%s"), self._position - length - 4, rc, length, data);
	...
	function handleBoundSheet(data){
        ...
        deriveEncoding();
        if (DEBUG)
            console.log(util.format("BOUNDSHEET: bv=%d data %s", bv, data));
		var sheetName = '';
		var visibility = 0;
		var sheetType = 0;
		var absPos = 0;
        ...
        if (sheetType != comm.XL_BOUNDSHEET_WORKSHEET){
            ...
            if (DEBUG || self.verbosity >= 1)
                console.log(util.format("NOTE *** Ignoring non-worksheet data named %s (type 0x%02x = %s)",
    ...
    function handleWriteAccess(data){
        ...
        if (DEBUG)  
            console.log("WRITEACCESS: %d bytes; raw=%s %s\n", data.length, userNameIsRaw, str)

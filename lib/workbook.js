...
exports.openWorkbook = function(streamRead, options){
	var opts = options || {};//todo: default 값으로 병합
	var interObj = {}
	var bk = new Workbook(interObj);

	return interObj.loadBiffN(streamRead)
    .then(function(){
        return interObj.getBOF(comm.XL_WORKBOOK_GLOBALS)
        .then(function(biffVer){
            if(!biffVer){
            ...
            if(biffVer <= 40){
                ...
                return interObj.fakeGlobalsGetSheet();
            }else if(biffVer == 45){
                return interObj.parseGlobals()
                .then(function(){
                    if(opts.onDemand){
                        console.log('*** WARNING: on_demand is not supported for this Excel version.');
                        console.log('*** Setting on_demand to False.');
...
function Workbook(interObj){
	...
	//biff2 ~ 8
	function loadBiffN(origRead){
        return origRead.peek(0,8)
        .then(function(buf){
            var bsig = buf.toString('hex').toLowerCase();
		
            if(bsig == compDoc.SIGNATURE){
                var cd = new compDoc.CompDoc(origRead, self.streamLength);
                return cd.locateNamedStream('Workbook')
                .then(function(streamRead){
                    self.streamRead = streamRead;
                    if(!self.streamRead || !self.streamRead.hasNext()){
                        return cd.locateNamedStream('Book')
                        .then(function(streamRead){
                            self.streamRead = streamRead;
                            if(!self.streamRead || !self.streamRead.hasNext()){
	...
	function getRecordPartsConditional(reqRecord){
        return self.streamRead.next(4)
        .then(function(buf){
            var code = buf.readUInt16LE(0) ;
            var length = buf.readUInt16LE(2);
            if (code != reqRecord)
                return [null, 0, ''];
            else
                return self.streamRead.next(length)
                .then(function(data){
	...
	function getSheet(sheetIndex, updatePos){
		...
		return getBOF(comm.XL_WORKSHEET)
        .then(function(_unusedBiffVersion){
	...
	function getSheets(){
        var DEBUG = 0
        var loop = function(sheetIdx){
            if(sheetIdx >= _sheetNames.length ) return 
            if (DEBUG) 
                console.log(
                    "GET_SHEETS: sheetIdx = %s, name = %s, pos = %s"
                    , sheetIdx, _sheetNames[sheetIdx], _shAbsPosList[sheetIdx]
                );
            return getSheet(sheetIdx++)
            .then(
                return loop(sheetIdx);
            });
        }
        return loop(0);
	}
	...
	function getBOF(reqStream){
		// DEBUG = 1
		// if DEBUG: print >> self.logfile, "getbof(): position", self._position
		//if DEBUG: print("reqd: 0x%04x" % reqStream, file=self.logfile)
        ...
        .then(function(opcd){
            ...
            if(comm.bofCodes.indexOf(opcode)<0){
                ...
                bofError('Expected BOF record; found 0x%s', buf.toString('hex') );
            }
            return get2Bytes();
        }).then(function(length, readLen){
            if( length == _EOF)
                bofError('Incomplete BOF record[1]; met end of file')
            if( ! (4 <= length &&  length <= 20)){
                var buf = new Buffer(4);
                buf.writeUInt32LE(opcode,0);
                bofError(util.format('Invalid length (%d) for BOF record type 0x%s', length, buf.toString('hex')));
            }
            var padding = new Buffer(Math.max(0, comm.bofLen[opcode] - length));
            padding.fill(0);
            return self.streamRead.next(length) //Type Buffer
            .then(function(data, len){
                ...
                */
                ...
                if ((reqStream == comm.XL_WORKBOOK_GLOBALS && gotGlobals) || streamType == reqStream) return version;
	...
	function handleSST(data){
        ...
        var loop = function(){
            var pms = getRecordPartsConditional(comm.XL_CONTINUE)
            .then(function(rc){
